...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef<WaveUniforms>({
    ...
    waveColor: new THREE.Uniform(new THREE.Color(waveR, waveG, waveB)),
  ...
  // When using `frameloop="demand"` (divider mode), make sure updates still render.
  useEffect(() => {
    invalidate();
  }, [
    invalidate,
...
export function Dither({
  ...
  return (
    <Canvas
      ...
      dpr={1}
      frameloop={
        disableAnimation && !enableMouseInteraction ? "demand" : "always"
      }
      gl={{
        antialias: false,
